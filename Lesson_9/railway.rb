  ...
  def edit_train
    loop do
      ...
      when 3
        ...
        if @stations.key?(station_number)
      ...
      when 4
        ...
        if train.type == :passenger
          train.met { |p_wagon, index| puts "Номер вагона #{index}, тип вагона #{p_wagon.type}, количество свободных мест: #{p_wagon.free_places}, количество занятых мест #{p_wagon.taken_places}" }
        elsif train.type == :cargo
          train.met { |c_wagon, index| puts "Номер вагона #{index}, тип вагона #{c_wagon.type}, количество свободных мест: #{c_wagon.free_space}, количество занятого места #{c_wagon.taken_space}" }
        end
